...
def cache(data,direc,method="a+"):
    with open(direc+".csv", method) as f:
        write = writer(f) 
...
def cache_price():
    ...
    prices_btc = []
    data_btc = get_json("https://api.cryptowat.ch/markets/kraken/btcusd/ohlc?periods="+str(period)+"&after="+str(last))
    data_btc = list(data_btc['result'][str(period)])[1:]
    prices_xmr = []
    data_xmr = get_json("https://api.cryptowat.ch/markets/kraken/xmrbtc/ohlc?periods="+str(period)+"&after="+str(last))
...
def cache_reddit():
    blocks = get_csv("reddit")
    last = blocks[-1][0]
    blocks = [x for x in blocks if x[0] < last - 6*31*24*60*60]
    last = blocks[-1][0]
    new = scrape(last)[::-1]
    cache(blocks + new,"reddit","w")
